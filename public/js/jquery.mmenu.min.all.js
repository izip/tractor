!function (a) {
    ...
    function k(c) {
        return"object" != typeof c && (c = {}), "undefined" != typeof c.panelNodeType && (a[b].deprecated("panelNodeType configuration option", "panelNodetype"), c.panelNodetype = c.panelNodeType), c = a.extend(!0, {}, a[b].configuration, c), "string" != typeof c.pageSelector && (c.pageSelector = "> " + c.pageNodetype), c
    }
    ...
    function m(b, c) {
        ...
        var d = a("." + e.panel, c), f = d.filter("." + e.current);
    ...
    var b = "mmenu", c = "4.0.4";
    if (!a[b]) {
        var d = {$wndw: null, $html: null, $body: null, $page: null, $blck: null, $allMenus: null, $scrollTopNode: null}, e = {}, f = {}, g = {}, h = 0;
        a[b] = function (a, b, c) {
            ...
            }, this.conf.transitionDuration), d.$html.addClass(e.opening), this.$menu.trigger(f.opening), window.scrollTo(0, 1)
        }, close: function () {
            var a = this;
            ...
            }, this.conf.transitionDuration), d.$html.removeClass(e.opening), this.$menu.trigger(f.closing), "close"
        }, _init: function () {
            if (this.opts = j(this.opts, this.conf, this.$menu), this.direction = this.opts.slidingSubmenus ? "horizontal" : "vertical", this._initPage(d.$page), this._initMenu(), this._initBlocker(), this._initPanles(), this._initLinks(), this._initOpenClose(), this._bindCustomEvents(), a[b].addons)for (var c = 0; c < a[b].addons.length; c++)"function" == typeof this["_addon_" + a[b].addons[c]] && this["_addon_" + a[b].addons[c]]()
        }, _bindCustomEvents: function () {
            var b = this;
            this.$menu.off(f.open + " " + f.close + " " + f.setPage).on(f.open + " " + f.close + " " + f.setPage, function (a) {
                a.stopPropagation()
            }), this.$menu.on(f.open,function (c) {
                return a(this).hasClass(e.current) ? (c.stopImmediatePropagation(), !1) : b.open()
            }).on(f.close,function (c) {
                return a(this).hasClass(e.current) ? b.close() : (c.stopImmediatePropagation(), !1)
            }).on(f.setPage, function (a, c) {
                b._initPage(c), b._initOpenClose()
            });
            var c = this.$menu.find(this.opts.isMenu && "horizontal" != this.direction ? "ul, ol" : "." + e.panel);
            ...
            var b = this;
            this.__refactorClass(a("." + this.conf.listClass, this.$menu), "list"), this.opts.isMenu && a("ul, ol", this.$menu).not(".mm-nolist").addClass(e.list);
            var c = a("." + e.list + " > li", this.$menu);
            ...
            }), this.__refactorClass(a("." + this.conf.panelClass, this.$menu), "panel"), this.$menu.children().filter(this.conf.panelNodetype).add(this.$menu.find("." + e.list).children().children().filter(this.conf.panelNodetype)).addClass(e.panel);
            var d = a("." + e.panel, this.$menu);
            d.each(function (c) {
                var d = a(this), f = d.attr("id") || e.mm("m" + b.serialnr + "-p" + c);
                d.attr("id", f)
            }), d.find("." + e.panel).each(function () {
                var d = a(this), f = d.is("ul, ol") ? d : d.find("ul ,ol").first(), h = d.parent(), i = h.find("> a, > span"), j = h.closest("." + e.panel);
                if (d.data(g.parent, h), h.parent().is("." + e.list)) {
            ...
            if (d.each(function () {
                ...
                var i = a("." + e.list + " > li." + e.selected, this.$menu);
            ...
            var b = this, c = a("." + e.list + " > li > a", this.$menu).not("." + e.subopen).not("." + e.subclose).not('[rel="external"]').not('[target="_blank"]');
            c.off(f.click).on(f.click, function (c) {
                var g = a(this), h = g.attr("href");
            ...
            var b = this, c = this.$menu.attr("id");
        ...
        }, a[b].version = c, a[b].defaults = {position: "left", zposition: "back", moveBackground: !0, slidingSubmenus: !0, modal: !1, classes: "", onClick: {setSelected: !0}}, a[b].configuration = {preventTabbing: !0, panelClass: "Panel", listClass: "List", selectedClass: "Selected", labelClass: "Label", pageNodetype: "div", panelNodetype: "ul, ol, div", transitionDuration: 400}, function () {
            var c = window.document, d = window.navigator.userAgent, e = "ontouchstart"in c, f = "WebkitOverflowScrolling"in c.documentElement.style, g = function () {
            ...
            a[b].support = {touch: e, transition: g, oldAndroidBrowser: h, overflowscrolling: function () {
...
!function (a) {
    ...
    a[b].prototype["_addon_" + c] = function () {
        var e = this.opts[c], f = a[b]._c, g = a[b]._d, h = a[b]._e;
        f.add("counter noresults"), h.add("update"), "boolean" == typeof e && (e = {add: e, update: e}), "object" != typeof e && (e = {}), e = a.extend(!0, {}, a[b].defaults[c], e), e.count && (a[b].deprecated('the option "count" for counters, the option "update"'), e.update = e.count), this.__refactorClass(a("em." + this.conf.counterClass, this.$menu), "counter"), e.add && a("." + f.panel, this.$menu).each(function () {
            var b = a(this), c = b.data(g.parent);
            if (c) {
                var d = a('<em class="' + f.counter + '" />'), e = c.find("> a." + f.subopen);
                e.parent().find("em." + f.counter).length || e.before(d)
            }
        }), e.update && a("em." + f.counter, this.$menu).each(function () {
            var b = a(this), c = a(b.next().attr("href"), this.$menu);
    ...
    }, a[b].defaults[c] = {add: !1, update: !1}, a[b].configuration.counterClass = "Counter", a[b].addons = a[b].addons || [], a[b].addons.push(c)
}(jQuery);
...
!function (a) {
    ...
    var b = "mmenu", c = "dragOpen";
    a[b].prototype["_addon_" + c] = function () {
        var e = this, f = this.opts[c];
        if (a.fn.hammer) {
            var g = a[b]._c, i = (a[b]._d, a[b]._e);
            g.add("dragging"), i.add("dragleft dragright dragup dragdown dragend");
            var j = a[b].glbl;
            if ("boolean" == typeof f && (f = {open: f}), "object" != typeof f && (f = {}), "number" != typeof f.maxStartPos && (f.maxStartPos = "left" == this.opts.position || "right" == this.opts.position ? 150 : 50), f = a.extend(!0, {}, a[b].defaults[c], f), f.open) {
                var k = 0, l = !1, m = 0, n = 0, o = "width";
                ...
                switch (this.opts.position) {
                    case"left":
                        var p = {events: i.dragleft + " " + i.dragright, open_dir: "right", close_dir: "left", delta: "deltaX", page: "pageX", negative: !1};
                        break;
                    case"right":
                        var p = {events: i.dragleft + " " + i.dragright, open_dir: "left", close_dir: "right", delta: "deltaX", page: "pageX", negative: !0};
                        break;
                    case"top":
                        var p = {events: i.dragup + " " + i.dragdown, open_dir: "down", close_dir: "up", delta: "deltaY", page: "pageY", negative: !1};
                        break;
                    case"bottom":
                        var p = {events: i.dragup + " " + i.dragdown, open_dir: "up", close_dir: "down", delta: "deltaY", page: "pageY", negative: !0}
                }
    ...
    }, a[b].defaults[c] = {open: !1, threshold: 50}, a[b].configuration[c] = {width: {perc: .8, min: 140, max: 440}, height: {perc: .8, min: 140, max: 880}}, a[b].addons = a[b].addons || [], a[b].addons.push(c)
}(jQuery);
...
!function (a) {
    ...
    a[b].prototype["_addon_" + c] = function () {
        var d = this, e = this.opts[c], f = this.conf[c];
        if ("horizontal" == this.direction) {
            var g = a[b]._c, i = (a[b]._d, a[b]._e);
            g.add("header hasheader prev next"), i.add("setheader");
            var j = a[b].glbl;
            "boolean" == typeof e && (e = {add: e, update: e}), "object" != typeof e && (e = {}), e = a.extend(!0, {}, a[b].defaults[c], e), e.add && a('<div class="' + g.header + '" />').prependTo(this.$menu).append('<a class="' + g.prev + '" href="#"></a>').append("<span></span>").append('<a class="' + g.next + '" href="#"></a>');
            var k = a("div." + g.header, this.$menu);
            if (k.length && this.$menu.addClass(g.hasheader), e.update && k.length) {
                var l = k.find("span"), m = k.find("." + g.prev), n = k.find("." + g.next), o = "#" + j.$page.attr("id");
                m.add(n).on(i.click, function (b) {
                    ...
                    var b = a(this), c = a("." + f.panelHeaderClass, b).text(), d = a("." + f.panelPrevClass, b).attr("href"), h = a("." + f.panelNextClass, b).attr("href");
    ...
    }, a[b].defaults[c] = {add: !1, update: !1, title: "Menu"}, a[b].configuration[c] = {panelHeaderClass: "Header", panelNextClass: "Next", panelPrevClass: "Prev"}, a[b].addons = a[b].addons || [], a[b].addons.push(c)
}(jQuery);
...
!function (a) {
    ...
    var b = "mmenu", c = "searchfield";
    a[b].prototype["_addon_" + c] = function () {
        var e = this, f = this.opts[c], g = a[b]._c, h = a[b]._d, i = a[b]._e;
        if (g.add("search hassearch noresults nosubresults counter"), i.add("search reset keyup change update"), "boolean" == typeof f && (f = {add: f, search: f}), "object" != typeof f && (f = {}), f = a.extend(!0, {}, a[b].defaults[c], f), f.add && (a('<div class="' + g.search + '" />').prependTo(this.$menu).append('<input placeholder="' + f.placeholder + '" type="text" autocomplete="off" />'), f.noResults && a("ul", this.$menu).first().append('<li class="' + g.noresults + '">' + f.noResults + "</li>")), a("div." + g.search, this.$menu).length && this.$menu.addClass(g.hassearch), f.search) {
            var j = a("div." + g.search, this.$menu).find("input");
            if (j.length) {
                var k = a("ul." + g.list + "> li." + g.label, this.$menu), l = a("ul." + g.list + "> li", this.$menu).not("." + g.subtitle).not("." + g.label).not("." + g.noresults), m = "> a";
                f.showLinksOnly || (m += ", > span"), j.on(i.keyup,function (a) {
                    d(a.keyCode) || e.$menu.trigger(i.search)
                }).on(i.change, function () {
                    e.$menu.trigger(i.search)
                }), this.$menu.on(i.reset + " " + i.search,function (a) {
                    a.stopPropagation()
                }).on(i.reset,function () {
                    e.$menu.trigger(i.search, [""])
                }).on(i.search, function (b, c) {
                    "string" == typeof c ? j.val(c) : c = j.val(), c = c.toLowerCase(), l.add(k).addClass(g.hidden), l.each(function () {
                        var b = a(this);
                        a(m, b).text().toLowerCase().indexOf(c) > -1 && b.add(b.prevAll("." + g.label).first()).removeClass(g.hidden)
                    }), a(a("." + g.panel, e.$menu).get().reverse()).each(function () {
                        var b = a(this), c = b.data(h.parent);
                        if (c) {
                            var d = b.add(b.find("> ." + g.list)).find("> li").not("." + g.subtitle).not("." + g.label).not("." + g.hidden);
                            d.length ? c.removeClass(g.hidden).removeClass(g.nosubresults).prevAll("." + g.label).first().removeClass(g.hidden) : (b.hasClass(g.current) && c.trigger(i.open), c.addClass(g.nosubresults))
                        }
                    }), e.$menu[l.not("." + g.hidden).length ? "removeClass" : "addClass"](g.noresults), a("em." + g.counter, e.$menu).trigger(i.update)
                })
            }
        }
    }, a[b].defaults[c] = {add: !1, search: !1, showLinksOnly: !0, placeholder: "Search", noResults: "No results found."}, a[b].addons = a[b].addons || [], a[b].addons.push(c)
}(jQuery);